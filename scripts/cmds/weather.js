...
module.exports = {
	...
	onStart: async function ({ args, message, envGlobal, getLang }) {
		...
		try {
			...
			const errorMessage = err.response?.data?.Message || "Unknown error occurred while fetching location.";
		...
		try {
			...
			const errorMessage = err.response?.data?.Message || "Unknown error occurred while fetching weather data.";
			return message.reply(getLang("error", errorMessage));
		}
