module.exports = {
  config: {
    ...
    countDown: 20,
    role: 0,
    description: "Sends a random anime meme video",
    category: "fun",
    guide: "{p}{n}animeme",
  },
  ...
  onStart: async function ({ api, event, message }) {
    ...
    if (availableVideos.length === 0) {
      this.sentVideos = [];
      return message.reply("All videos have been sent. Please try again later.");
    }
    ...
    try {
      ...
      message.reply("Failed to send the anime meme. Please try again.");
    }
  },

  addVideo: function (link) {
    if (!this.sentVideos.includes(link)) {
      this.sentVideos.push(link);
    }
  },
